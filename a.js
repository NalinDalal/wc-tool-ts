...
function printResults(results, countBytes, countLines, countWords, countChars, filename) {
    let output = "";
    if (countLines)
        output += `${results.lineCount} `;
    if (countWords)
        output += `${results.wordCount} `;
    if (countBytes)
        output += `${results.byteCount} `;
    if (countChars)
        output += `${results.charCount} `;
    console.log(output + (filename ? filename : ""));
}
const args = process.argv.slice(2);
let countBytes = false, countLines = false, countWords = false, countChars = false;
let filename = "";
...
if (filename) {
    fs.readFile(filename, "utf-8", (err, data) => {
        ...
        printResults(results, countBytes, countLines, countWords, countChars, filename);
    });
}
else {
    let input = "";
